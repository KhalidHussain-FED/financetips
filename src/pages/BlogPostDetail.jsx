...
export default function BlogPostDetail() {
  ...
  return (
    <div className="bg-gray-50 min-h-screen">
      {/* Hero Header */}
      <div className="relative text-white py-16 sm:py-20 overflow-hidden">
        <img
          src={post.featured_image || "https://images.unsplash.com/photo-1554224155-6726b3ff858f?w=1600&q=80"}
          alt={post.title}
        ...
        <div className="max-w-3xl mx-auto px-4 sm:px-6 relative z-10">
          {/* Breadcrumb */}
          <nav className="text-xs text-slate-300 mb-6 flex items-center gap-1.5 flex-wrap">
          ...
          {/* Category Badge */}
          {post.category && (
            <span className={`inline-block px-3 py-1 rounded-full text-xs font-bold mb-5 ${catColor}`}>
              {post.category}
            </span>
          )}

          {/* Title */}
          <h1 className="font-heading text-2xl sm:text-3xl lg:text-4xl font-bold leading-tight mb-6">
            {post.title}
          </h1>
      ...
      <div className="max-w-3xl mx-auto px-4 sm:px-6 py-10">
            ...
            <div className="flex flex-wrap items-center gap-2">
              ...
              {post.tags.split(',').map(tag => (
                <Link
                  ...
                  className="px-3 py-1.5 bg-white border border-gray-200 text-xs rounded-full hover:bg-blue-50 hover:border-blue-300 hover:text-blue-700 transition-colors text-slate-600"
                >
                  {tag.trim()}
                </Link>
              ))}
        ...
        <div className="mt-8 pt-6 border-t border-gray-200">
          <div className="flex flex-wrap items-center justify-between gap-4">
            {/* Social Share */}
            <div className="flex flex-wrap items-center gap-2">
              <span className="flex items-center gap-1.5 text-sm font-semibold text-slate-600 mr-1">
              ...
              {(() => {
                ...
                return (
                  <div className="flex flex-wrap gap-2">
                    <a href={`https://www.facebook.com/sharer/sharer.php?u=${url}`} target="_blank" rel="noopener noreferrer" className="flex items-center gap-1.5 px-3 py-1.5 rounded-full text-xs font-semibold text-white transition-opacity hover:opacity-90" style={{ backgroundColor: '#1877F2' }}>
                    ...
                    <a href={`https://twitter.com/intent/tweet?url=${url}&text=${title}`} target="_blank" rel="noopener noreferrer" className="flex items-center gap-1.5 px-3 py-1.5 rounded-full text-xs font-semibold text-white transition-opacity hover:opacity-90 bg-black">
                    ...
                    <a href={`https://www.linkedin.com/sharing/share-offsite/?url=${url}`} target="_blank" rel="noopener noreferrer" className="flex items-center gap-1.5 px-3 py-1.5 rounded-full text-xs font-semibold text-white transition-opacity hover:opacity-90" style={{ backgroundColor: '#0A66C2' }}>
                      <svg className="w-3 h-3" fill="currentColor" viewBox="0 0 24 24"><path d="M20.447 20.452h-3.554v-5.569c0-1.328-.027-3.037-1.852-3.037-1.853 0-2.136 1.445-2.136 2.939v5.667H9.351V9h3.414v1.561h.046c.477-.9 1.637-1.85 3.37-1.85 3.601 0 4.267 2.37 4.267 5.455v6.286zM5.337 7.433a2.062 2.062 0 01-2.063-2.065 2.064 2.064 0 112.063 2.065zm1.782 13.019H3.555V9h3.564v11.452zM22.225 0H1.771C.792 0 0 .774 0 1.729v20.542C0 23.227.792 24 1.771 24h20.451C23.2 24 24 23.227 24 22.271V1.729C24 .774 23.2 0 22.222 0h.003z"/></svg>
                      LinkedIn
                    </a>
                    <a href={`https://wa.me/?text=${title}%20${url}`} target="_blank" rel="noopener noreferrer" className="flex items-center gap-1.5 px-3 py-1.5 rounded-full text-xs font-semibold text-white transition-opacity hover:opacity-90" style={{ backgroundColor: '#25D366' }}>
                    ...
                    <a href={`mailto:?subject=${title}&body=Check out this article: ${url}`} className="flex items-center gap-1.5 px-3 py-1.5 rounded-full text-xs font-semibold text-white transition-opacity hover:opacity-90 bg-slate-600">
